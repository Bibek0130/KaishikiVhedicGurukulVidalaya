import useInView from '../Count Dashboard/useInView'
import useCounter from '../Count Dashboard/useCounter'
...
export default function StatCard({ stat, index }) {
    ...
    return (
        ...
        >
            <div style={{ fontSize: "2.4rem", marginBottom: ".6rem" }}>{stat.icon}</div>
            <div style={{ fontFamily: "'EB Garamond', serif", fontSize: "3rem", fontWeight: 500, color: T.saff, lineHeight: 1 }}>
                {count}{stat.suffix}
            </div>
            <div style={{ fontFamily: "'EB Garamond', serif", fontSize: "1.15rem", color: T.ink, marginTop: ".4rem", fontWeight: 400 }}>
                {stat.label}
            </div>
            <div style={{ fontFamily: "'Hind', sans-serif", fontSize: ".82rem", color: T.inkSoft, marginTop: ".25rem", lineHeight: 1.5 }}>
                {stat.desc}
            </div>
        </div>
    );
